--[[ KICKSTART.NVIM
  ...
  Basics of Lua: https://learnxinyminutes.com/docs/lua/
  ...
  Once you understand Neovim basics, read through and understand your init.lua file.

  Next, run AND READ `:help`. It contains basic information about reading, navigating and searching
  the builtin help documentation. This should be the first place you go to look when you're confused.

  Use the keymap "<leader>sh" to [S]earch the [H]elp documentation.
  There are also several `:help X` comments throughout the init.lua file.

  If you experience any errors while trying to install kickstart.nvim, run `:checkhealth` for more info.
...
--   This setting is scheduled after `UiEnter` to not increase startup-time.
--   See `:help 'clipboard'`
...
-- Disable arrow keys in normal mode (l2p n00b)
...
--[[ PLUGINS
  ...
  Configure and install plugins for lazy.nvim
  ...
  For example, in the which-key configuration, we use `event = 'VimEnter'`
  which loads which-key before all the UI elements are loaded. Events can be
  normal autocommands events (`:help autocmd-events`). Then because use
  the `opts` key (recommended), the configuration runs after the plugin
  has been loaded as `require(MODULE).setup(opts)`.
...
require("lazy").setup({
	...
	-- Show pending keybinds (e.g. after pressing <leader>)
	--   See `:help which-key`
	{
    ...
    --]]
      ...
      --]]
			...
			require("mason-lspconfig").setup({
				ensure_installed = {}, -- This is explicitly set to an empty table (Kickstart populates installs via mason-tool-installer)
	...
	-- Autocompletions
	{
		...
		opts = {
			keymap = {
				...
				--   Especially to understand why the 'default' preset is recommended
				--
  ...
  --]]
	...
	-- Collection of various small independent plugins/modules
	{
		...
		config = function()
			-- Better Around/Inside (a/i) text objects
			--
			-- Examples:
			...
			--   You could replace this with another statusline plugin you prefer
